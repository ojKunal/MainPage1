import { FunctionComponent } from "react";
import FrameComponent4 from "./FrameComponent4";
import DivcnHpNexttripSlider from "./DivcnHpNexttripSlider";
import ScrollableImages from "./ScrollableImage";

export type FrameComponent3Type = {
  className?: string;
};

const FrameComponent3: FunctionComponent<FrameComponent3Type> = ({
  className = "",
}) => {
  return (
    <section
      className={`self-stretch flex flex-row items-start justify-center pt-0 px-10 pb-[94.9px] box-border max-w-full shrink-0 text-left text-4xl text-gray-300 font-poppins mq900:pb-[26px] mq900:box-border mq1275:pb-10 mq1275:box-border overflow-x-hidden overflow-hidden ${className}`}
    >
      <div className="w-[1476.1px] flex flex-col items-start justify-start pt-0 px-0  box-border  max-w-full mq450:gap-[19px] mq900:h-auto mq900:gap-[38px] mq900:pb-[137px] mq900:box-border mq1275:pb-[211px] mq1275:box-border">
        {/* Recent Searches start */}

        <div className="self-stretch flex flex-col items-start justify-start gap-[17.2px] max-w-full shrink-0">
          <h3 className="m-0 relative text-inherit tracking-[-0.46px] leading-[23px] font-bold font-inherit mq450:text-lg mq450:leading-[18px]">
            Recent searches
          </h3>
          <div className="self-stretch flex flex-col items-start justify-start gap-[1.8px] max-w-full text-lg text-gray-100">
            <div className="relative leading-[22px]">
              Continue where you left off
            </div>
            <div className="self-stretch h-[116px] overflow-hidden shrink-0 flex flex-col items-start justify-start pt-0 px-0 pb-[18.9px] box-border gap-[21.9px] max-w-full text-base text-gray-300 mq900:h-auto">
              <div className="w-[546px] flex flex-col items-start justify-start gap-[1px] shrink-0 max-w-full">
                <div className="w-px h-6 relative inline-block">{` `}</div>

                {/* ................................................ */}

                <div className="self-stretch flex flex-row flex-wrap items-start justify-start gap-[15px] text-sm text-black font-inter">
                  <div className="flex-1 rounded-21xl  shadow-lg box-border overflow-x-auto flex flex-row items-end justify-start pt-4 px-4 pb-[18px] gap-[8px] min-w-[202px] border-[0.6px] border-solid border-gainsboro">
                    <div className="flex flex-col items-start justify-end pt-0 px-0 pb-[4.8px]">
                      <img
                        className="w-6 h-6 relative overflow-hidden shrink-0"
                        alt=""
                        src="/img.svg"
                      />
                    </div>
                    <div className="flex flex-col items-start justify-start py-0 pr-1.5 pl-0 gap-[7.2px]">
                      <div className="h-[17px] overflow-hidden shrink-0 flex flex-col items-start justify-start pt-[15px] px-0 pb-px box-border">
                        <div className="mt-[-17px] flex flex-row items-start justify-start gap-[8.2px] shrink-0">
                          <div className="relative leading-[17px] font-semibold">
                            Shalem Miracle Ministries
                          </div>
                          <div className="relative leading-[17px] font-semibold inline-block min-w-[5px]">
                            ,
                          </div>
                        </div>
                        <div className="w-[35px] relative leading-[17px] font-poppins inline-block shrink-0">
                          India
                        </div>
                      </div>
                      <div className="w-[143.7px] flex flex-row items-start justify-start gap-[2.8px] text-xs text-gray-100 font-poppins">
                        <div className="flex-1 flex flex-row items-start justify-start">
                          <div className="flex flex-col items-start justify-start py-0 px-0">
                            <div className="mr-[-0.4px] relative leading-[12px] inline-block min-w-[34px] shrink-0 whitespace-nowrap">
                              27 Jul
                            </div>
                          </div>
                          <div className="relative leading-[12px] inline-block min-w-[10px] shrink-0">{` - `}</div>
                        </div>
                        <div className="flex-1 flex flex-row items-start justify-start">
                          <div className="flex flex-col items-start justify-start py-0 px-0">
                            <div className="mr-[-0.3px] relative leading-[12px] inline-block min-w-[35px] shrink-0 whitespace-nowrap">
                              28 Jul
                            </div>
                          </div>
                          <div className="relative leading-[12px] inline-block min-w-[9px] shrink-0">{` • `}</div>
                        </div>
                        <div className="relative leading-[12px] inline-block min-w-[7px]">
                          2
                        </div>
                        <div className="flex-1 relative leading-[12px] inline-block min-w-[41px]">
                          guests
                        </div>
                      </div>
                    </div>
                    <div className="flex flex-col items-start justify-end pt-0 px-0 pb-[10.5px]">
                      <img
                        className="w-3 h-[10.5px] relative overflow-hidden shrink-0"
                        alt=""
                        src="/frame-7.svg"
                      />
                    </div>
                  </div>

                  {/* ................................. */}

                  <div className="w-[243px] shadow-lg rounded-21xl box-border overflow-x-auto shrink-0 flex flex-row items-end justify-start pt-4 px-4 pb-[18px] gap-[8px] border-[0.6px] border-solid border-gainsboro">
                    <div className="flex flex-col items-start justify-end pt-0 px-0 pb-[4.8px]">
                      <img
                        className="w-6 h-6 relative overflow-hidden shrink-0"
                        alt=""
                        src="/img-1.svg"
                      />
                    </div>
                    <div className="w-[149.9px] shrink-0 flex flex-col items-start justify-start py-0 pr-1.5 pl-0 box-border gap-[7.2px]">
                      <div className="self-stretch h-[17px] overflow-hidden shrink-0 flex flex-row items-start justify-start pt-0 px-0 pb-[1.7px] box-border">
                        <div className="mt-[-1.7px] relative leading-[17px] font-semibold inline-block min-w-[33px]">
                          India
                        </div>
                      </div>
                      <div className="self-stretch flex flex-row items-start justify-start gap-[2.8px] text-xs text-gray-100 font-poppins">
                        <div className="flex-1 flex flex-row items-start justify-start">
                          <div className="flex flex-col items-start justify-start py-0 px-0">
                            <div className="mr-[-0.3px] relative leading-[12px] inline-block min-w-[34px] shrink-0 whitespace-nowrap">
                              27 Jul
                            </div>
                          </div>
                          <div className="relative leading-[12px] inline-block min-w-[10px] shrink-0">{` - `}</div>
                        </div>
                        <div className="flex-1 flex flex-row items-start justify-start">
                          <div className="flex flex-col items-start justify-start py-0 px-0">
                            <div className="mr-[-0.3px] relative leading-[12px] inline-block min-w-[35px] shrink-0 whitespace-nowrap">
                              28 Jul
                            </div>
                          </div>
                          <div className="relative leading-[12px] inline-block min-w-[9px] shrink-0">{` • `}</div>
                        </div>
                        <div className="relative leading-[12px] inline-block min-w-[7px]">
                          2
                        </div>
                        <div className="flex-1 relative leading-[12px] inline-block min-w-[41px]">
                          guests
                        </div>
                      </div>
                    </div>
                    <div className="flex flex-col items-start justify-end pt-0 px-0 pb-[10.5px]">
                      <img
                        className="w-3 h-[10.5px] relative overflow-hidden shrink-0"
                        alt=""
                        src="/frame-8.svg"
                      />
                    </div>
                  </div>
                </div>
              </div>
              <div className="w-px relative inline-block shrink-0">{` `}</div>
              <div className="self-stretch h-[18px] relative bg-white overflow-hidden shrink-0 hidden" />
            </div>
          </div>
        </div>

        {/* Recent Searches end */}

        {/* Deals Part Start  */}

        {/* <div className="self-stretch flex flex-col items-start justify-start pt-0 px-0 pb-[19.9px] box-border gap-[1px] max-w-full shrink-0 text-base">
          <div className="self-stretch h-14 overflow-hidden shrink-0 flex flex-row items-start justify-between pt-0 px-0 pb-7 box-border relative max-w-full gap-[20px] mq900:h-auto">
            <div className="h-0 w-px absolute !m-[0] top-[0px] left-[0px] inline-block">{` `}</div>
            <div className="mt-[-1.7px] w-[667.4px] flex flex-col items-start justify-start gap-[35.7px] shrink-0 max-w-[calc(100%_-_86px)] text-lg mq900:gap-[18px]">
              <div className="self-stretch flex flex-row items-start justify-start gap-[39px] mq900:flex-wrap mq900:gap-[19px]">
                <div className="flex flex-col items-start justify-start py-0 pr-2 pl-0 font-inter">
                  <div className="relative leading-[22px] font-semibold inline-block min-w-[126px] z-[1]">
                    Ongoing deals
                  </div>
                </div>
                <div className="relative leading-[22px]">
                  Discounted weekly stays
                </div>
                <div className="relative leading-[22px]">
                  Discounted monthly stays
                </div>
              </div>
              <div className="w-px relative text-base inline-block">{` `}</div>
            </div>
          </div>


          <div className="self-stretch h-[603px] overflow-hidden shrink-0 flex flex-row items-start justify-start pt-0 px-0 pb-[30px] box-border gap-[299px] max-w-full mq450:gap-[37px] mq900:gap-[75px] mq1275:gap-[149px] mq1600:h-auto"> */}
            {/* <div className=" flex flex-col items-start justify-start  shrink-0 ">
              <div className="w-px h-0 relative inline-block">{` `}</div>
              <div className="max-w-full flex     text-4xl text-black "> */}
                {/* <div className="flex-1 flex flex-col items-start justify-start gap-[16.8px] min-w-[348px] max-w-full mq450:min-w-full">
                  <img
                    className="self-stretch h-[492.7px] relative rounded-2xl max-w-full overflow-hidden shrink-0 object-cover"
                    loading="lazy"
                    alt=""
                    src="/divcn--cardcn--card7@2x.png"
                  />
                  <div className="w-[245.5px] flex flex-col items-start justify-start gap-[6.5px]">
                    <h3 className="m-0 relative text-inherit leading-[28px] font-bold font-inherit inline-block min-w-[111px] mq450:text-lg mq450:leading-[22px]">
                      New York
                    </h3>
                    <div className="self-stretch flex flex-row items-start justify-start text-base">
                      <div className="flex flex-row items-start justify-start gap-[3.5px]">
                        <div className="relative tracking-[0.16px] leading-[24px]">{`Deals starting at `}</div>
                        <div className="relative tracking-[0.16px] leading-[24px] font-semibold font-inter inline-block min-w-[107px]">
                          €73 per night
                        </div>
                      </div>
                      <div className="w-px flex flex-col items-start justify-start pt-[54.3px] px-0 pb-0 box-border ml-[-245.5px] text-gray-300">
                        <div className="self-stretch relative">{` `}</div>
                      </div>
                    </div> */}
                  {/* </div>
                </div> */}
                {/* <FrameComponent4
                  divcnCardcnCard7="/divcn--cardcn--card7-1@2x.png"
                  tokyo="Tokyo"
                  perNight="€48 per night"
                />
                <FrameComponent4
                  divcnCardcnCard7="/divcn--cardcn--card7-2@2x.png"
                  tokyo="Dubai"
                  perNight="€49 per night"
                  propFlex="1"
                  propMinWidth="348px"
                  propAlignSelf="unset"
                  propWidth="unset"
                />
                <FrameComponent4
                  divcnCardcnCard7="/divcn--cardcn--card7-3@2x.png"
                  tokyo="London"
                  perNight="€115 per night"
                  propFlex="1"
                  propMinWidth="348px"
                  propAlignSelf="unset"
                  propWidth="unset"
                />
                <FrameComponent4
                  divcnCardcnCard7="/divcn--cardcn--card7-4@2x.png"
                  tokyo="Melbourne"
                  perNight="€62 per night"
                  propFlex="1"
                  propMinWidth="348px"
                  propAlignSelf="unset"
                  propWidth="unset" */}
                {/* /> */}
              {/* </div>
            </div> */}









            
            {/* <div className="h-[573px] w-[1480px] flex flex-col items-start justify-start pt-[5px] px-0 pb-0 box-border max-w-full shrink-0 text-4xl text-black">
              <div className="w-[-299px] flex-1 overflow-hidden flex flex-row items-start justify-start gap-[20px] shrink-0">
                <FrameComponent4
                  divcnCardcnCard7="/divcn--cardcn--card7-5@2x.png"
                  tokyo="Singapore"
                  perNight="€53 per night"
                  propFlex="unset"
                  propMinWidth="unset"
                  propAlignSelf="stretch"
                  propWidth="355px"
                />
                <FrameComponent4
                  divcnCardcnCard7="/divcn--cardcn--card7-6@2x.png"
                  tokyo="Paris"
                  perNight="€73 per night"
                  propFlex="unset"
                  propMinWidth="unset"
                  propAlignSelf="stretch"
                  propWidth="355px"
                />
                <FrameComponent4
                  divcnCardcnCard7="/divcn--cardcn--card7-7@2x.png"
                  tokyo="Rio De Janeiro"
                  perNight="€23 per night"
                  propFlex="unset"
                  propMinWidth="unset"
                  propAlignSelf="stretch"
                  propWidth="355px"
                />
                <div className="self-stretch w-[355px] flex flex-col items-start justify-start gap-[16.8px] max-w-full shrink-0">
                  <div className="self-stretch flex-1 relative shadow-[0px_2px_2px_rgba(87,_75,_144,_0.15)] rounded-2xl bg-darkgray-200 overflow-hidden" />
                  <div className="w-[246.5px] h-[58.5px] flex flex-col items-start justify-start gap-[6.5px]">
                    <h3 className="m-0 w-[53px] relative text-inherit leading-[27.6px] font-bold font-inherit inline-block mq450:text-lg mq450:leading-[22px]">
                      Split
                    </h3>
                    <div className="self-stretch flex-1 flex flex-row items-start justify-start gap-[3.5px] text-base">
                      <div className="self-stretch flex-1 relative tracking-[0.16px] leading-[24px]">{`Deals starting at `}</div>
                      <div className="w-[108px] relative tracking-[0.16px] leading-[24px] font-semibold font-inter inline-block">
                        €35 per night
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div> */}



          {/* </div>
        </div> */}

        <ScrollableImages/>

        {/* Deals Part end */}

        {/* Property Page start */}

        <div className="self-stretch h-[817.2px] flex flex-col items-start justify-start pt-0 px-0 pb-[717.3px] box-border gap-[31.8px] max-w-full shrink-0 text-13xl font-inter mq900:gap-[16px] mq900:pb-[303px] mq900:box-border mq1275:pb-[466px] mq1275:box-border">
          <div className="w-[1466px] flex flex-row items-start justify-center py-0 px-5 box-border max-w-full shrink-0">
            <div className="w-[390px] flex flex-col items-start justify-start gap-[15.6px] shrink-0 max-w-full">
              <h1 className="m-0 relative text-inherit leading-[39px] font-semibold font-inherit mq450:text-lgi mq450:leading-[23px] mq900:text-7xl mq900:leading-[31px]">
                Explore by Property Type
              </h1>
              <div className="self-stretch flex flex-row items-start justify-end py-0 px-[62px] text-lg text-gray-100 font-poppins mq450:pl-5 mq450:pr-5 mq450:box-border">
                <div className="relative leading-[22px]">
                  So many Nests, so little time!
                </div>
              </div>
            </div>
          </div>
          <div className="self-stretch flex flex-wrap items-start justify-start gap-5 min-h-[686px] shrink-0  text-4xl text-black font-poppins">
            <DivcnHpNexttripSlider
              imgcnImageDefault="/imgcn--imagedefault@2x.png"
              hotels="Hotels"
              properties="1,506,710 properties"
            />
            <DivcnHpNexttripSlider
              imgcnImageDefault="/imgcn--imagedefault-1@2x.png"
              hotels="Apartments"
              properties="237,285 properties"
              propGap="unset"
            />
            <DivcnHpNexttripSlider
              imgcnImageDefault="/imgcn--imagedefault-2@2x.png"
              hotels="Vacation homes"
              properties="236,574 properties"
              propGap="unset"
            />
            <DivcnHpNexttripSlider
              imgcnImageDefault="/imgcn--imagedefault-3@2x.png"
              hotels={`B&Bs`}
              properties="148,854 properties"
              propGap="unset"
            />
            <DivcnHpNexttripSlider
              imgcnImageDefault="/imgcn--imagedefault-4@2x.png"
              hotels="Villas"
              properties="83,172 properties"
              propGap="0.1px"
            />
            <DivcnHpNexttripSlider
              imgcnImageDefault="/imgcn--imagedefault-5@2x.png"
              hotels="Bungalows"
              properties="54,770 properties"
              propGap="0.1px"
            />
            <DivcnHpNexttripSlider
              imgcnImageDefault="/imgcn--imagedefault-6@2x.png"
              hotels="Hostels"
              properties="41,610 properties"
              propGap="0.1px"
            />
            <DivcnHpNexttripSlider
              imgcnImageDefault="/imgcn--imagedefault-7@2x.png"
              hotels="Cabins"
              properties="4,792 properties"
              propGap="0.1px"
            />
          </div>
        </div>
        {/* Property Page end */}

        {/* Banner start */}

        <div className="mt-[400px] self-stretch rounded-xl bg-mediumturquoise flex flex-row items-start justify-between py-[30px] pr-[135px] pl-28 box-border max-w-full  gap-[20px] text-23xl text-white ">
          <img
            className="w-[572.2px] max-h-full overflow-hidden shrink-0 object-contain max-w-full mq1600:flex-1"
            loading="lazy"
            alt=""
            src="/frame-9@2x.png"
          />

          <div className="w-[529px] flex flex-col items-start justify-start pt-[92.1px] px-0 pb-0 box-border min-w-[529px] max-w-full mq900:min-w-full mq1600:flex-1">
            <div className="self-stretch flex flex-col items-start justify-start gap-[31.8px] mq900:gap-[16px]">
              <div className="self-stretch flex flex-col items-start justify-start gap-[9.6px]">
                <h1 className="m-0 text-inherit leading-[51px] font-bold font-inherit mq450:text-6xl mq450:leading-[30px] mq900:text-15xl mq900:leading-[40px]">
                  The CuddlyNest Promise
                </h1>
                <div className="self-stretch flex flex-row items-start justify-center py-0 px-5 text-lg">
                  <div className="leading-[22px] font-medium">{`Free cancellation & waived service fees`}</div>
                </div>
              </div>
              <div className="self-stretch flex flex-row items-start justify-center py-0 pr-[21px] pl-5">
                <button className="cursor-pointer [border:none] pt-[7.8px] px-[65px] pb-[8.2px] bg-[transparent] w-[200px] rounded-3xs [background:linear-gradient(92deg,_#f35597,_#7868c7_99.81%)] flex flex-row items-start justify-start box-border whitespace-nowrap">
                  <div className="text-sm tracking-[0.14px] leading-[21px] font-semibold font-inter text-white text-left inline-block min-w-[65px]">
                    See more
                  </div>
                </button>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default FrameComponent3;
